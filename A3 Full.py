...
#Check if "(config)#" was actually received
    ...
    if result != 0:
        print('--- Failed to create the Syslog access list')
        exit()
    else:
        print('Successfully created the Syslog access list!')
...
#
    ...
    if result != 0:
        print('--- Failed to allow tramission of traffic from this router')
        exit()
    else:
        print('Successfully allowed transmission of traffic from this router!')

#deny all ofhter traffic than what has already been permitted.
    session.sendline('deny ip any any')
    result = session.expect([r'\(config-ext-nacl\)#', pexpect.TIMEOUT, pexpect.EOF])


    if result != 0:
        print('--- Failed to deny all other traffic')
        exit()
    else:
        print('Successfully denied all other traffic!')

#set our router as the host so that we receive the log messages.        
...
#set the logging trap to 6 so that almost all messages are sent.
    session.sendline('logging trap informational')
    result = session.expect([r'\(config\)#', pexpect.TIMEOUT, pexpect.EOF])


    if result != 0:
        print('--- Failed to set the logging trap level to 6/7 (informational)')
        exit()
    else:
        print('Successfully set the logging trap level to 6/7 (informational)!')
